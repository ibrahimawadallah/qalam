  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="relative" ref={countryRef}>
        <input
          className="w-full rounded-lg border border-blue-500/20 bg-[#050a14] px-3 py-2 text-sm text-blue-100 placeholder:text-blue-300/40 focus:outline-none focus:border-blue-500/50"
        ...
        {showCountryList && filteredCountries.length > 0 && (
          <div className="absolute z-30 mt-1 max-h-52 w-full overflow-auto rounded-lg border border-blue-500/20 bg-[#0a1428] shadow-xl">
            {filteredCountries.map((item) => (
              <button
                ...
                className={`w-full px-3 py-2 text-left text-sm hover:bg-blue-500/10 ${
              ...
              >
                ...
                <span className="ml-2 text-xs text-blue-300/40">({item.cities.length} cities)</span>
              </button>
            ))}
          </div>
        )}
      ...
      <div className="relative" ref={cityRef}>
        <input
          className="w-full rounded-lg border border-blue-500/20 bg-[#050a14] px-3 py-2 text-sm text-blue-100 placeholder:text-blue-300/40 focus:outline-none focus:border-blue-500/50 disabled:opacity-50"
        ...
        {showCityList && matchedCountry && availableCities.length > 0 && (
          <div className="absolute z-30 mt-1 max-h-52 w-full overflow-auto rounded-lg border border-blue-500/20 bg-[#0a1428] shadow-xl">
            ...
            {availableCities.filter((c) => c.toLowerCase().includes(cityInput.toLowerCase().trim())).length === 0 && (
              <div className="px-3 py-2 text-xs text-blue-300/50">Type to search cities…</div>
            )}
          </div>
        )}
      </div>
